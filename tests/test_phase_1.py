...
async def test_ollama_recommended_models():
    ...
    from providers.local.ollama_engine import ModelSize, OllamaEngine
...
def test_json_memory_add_entry():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        entry_id = mem.add_entry(
            memory_type="test", content="Hello world", tags=["test"]
        )
...
def test_memory_manager_task_result():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        manager.add_task_result(
            task_name="code_gen", result="def hello(): pass", success=True
        )
