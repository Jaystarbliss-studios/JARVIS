...
from src.security.encryption import VoiceprintEncryption, AuditLogger, AntiSpoofing
from src.recognition.stt import OfflineSTT
from src.command.executor import CommandParser, CommandExecutor
...
class JarvisAssistant:
    ...
    def enroll_user(self) -> bool:
        ...
        voiceprint = enroller.run_enrollment(
            ...
            audio_capture_instance=self.audio
        )
    ...
    def verify_and_execute(self) -> None:
        ...
        if not is_valid:
            ...
            self.audit_logger.log_verification_attempt("FAIL", f"Audio invalid: {reason}")
            return
        ...
        if not command_config:
            ...
            print("Command not recognized. Say something like: 'time', 'open browser', 'play music'")
            return
        ...
        self.audit_logger.log_command_execution(
            command_config.get('description', text),
            success
        )
...
Examples:
  ...
  python main.py --interactive    # Continuous verification loop
    ...
    parser.add_argument(
        '--enroll',
        action='store_true',
        help='Enroll new user voice'
    )
    parser.add_argument(
        '--verify',
        action='store_true',
        help='Single verification attempt'
    )
    parser.add_argument(
        '--interactive',
        action='store_true',
        help='Continuous verification loop'
    )
    parser.add_argument(
        '--list-devices',
        action='store_true',
