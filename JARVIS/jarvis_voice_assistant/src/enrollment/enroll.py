...
class EnrollmentManager:
    ...
    def run_enrollment(
        self, num_samples: int = 7, audio_capture_instance=None
    ) -> np.ndarray | None:
        """
        ...
        print(
            f"Each sample should be approximately {self.sample_duration:.1f} seconds."
        )
        print("\nGuidelines:")
        ...
        for sample_idx in range(num_samples):
            ...
            try:
                audio = audio_capture_instance.record(
                    duration=self.sample_duration, show_progress=False
                )
        ...
        print("✓ Voiceprint created (512-dim speaker embedding)")
    ...
    def save_voiceprint(
        self, voiceprint: np.ndarray, filepath: str, encryptor_instance
    ) -> bool:
        """
        ...
        """
        try:
            success = encryptor_instance.save_encrypted_voiceprint(voiceprint, filepath)
    ...
    def get_enrollment_summary(self) -> dict:
        ...
        return {
            ...
            "embedding_stats": {
                "mean_norm": float(
                    np.mean([np.linalg.norm(e) for e in self.embeddings])
                ),
